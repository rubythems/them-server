...
module Gem
  module Server
    ...
    class AuthenticationApp < Roda
      ...
      plugin :render, escape: true, views: File.expand_path("../../app/templates/rodauth", __dir__), layout: false
      plugin :flash
      ...
      plugin :rodauth do
        ...
        # Ensure Rodauth generates absolute paths under /auth when mounted there
        prefix "/auth"

        # Temporarily disable CSRF checks to simplify integration
        check_csrf? false

        # Do not require password confirmation for account creation
        ...
        after_login do
          remember_login
          # Mirror values into the shared Rack session for Hanami actions to consume
          acct = account
          owner = db[:owners].where(account_id: acct[:id]).first
          if owner.nil?
            email = acct[:email]
            name = email
            now = Time.now
            oid = db[:owners].insert(
              name: name,
              public_key: nil,
              api_key: nil,
              account_id: acct[:id],
        ...
        login_redirect "/auth/dashboard"
        logout_redirect "/"
        create_account_redirect "/auth/login"
      ...
      route do |r|
        ...
        # # Forward dashboard to Hanami app, but require login first
        r.is "dashboard" do
          rodauth.require_account
          return Hanami.app.call(r.env)
        end
