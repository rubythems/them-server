...
# Start server 1
HANAMI_PORT=9292 HANAMI_ENV=development GEM_SERVER_DB="$GEM_SERVER_DB" \
...
# Start server 2
HANAMI_PORT=9393 HANAMI_ENV=development GEM_SERVER_DB="$GEM_SERVER_DB" \
  bundle exec rackup -p 9393 -E development --host 0.0.0.0 --daemonize

echo "Server 2 running on http://localhost:9393 using DB $GEM_SERVER_DB"
...
export FEDERATION_BASE_URL="http://localhost:9292"
ruby -e 'require "gem/server/federation_client"; Gem::Server::FederationClient.new.announce(to_base_url: "http://localhost:9393")'
ruby -e 'require "gem/server/federation_client"; Gem::Server::FederationClient.new.subscribe(to_base_url: "http://localhost:9393")'

# Announce 2 -> 1 and subscribe 1 to 2
export FEDERATION_BASE_URL="http://localhost:9393"
ruby -e 'require "gem/server/federation_client"; Gem::Server::FederationClient.new.announce(to_base_url: "http://localhost:9292")'
ruby -e 'require "gem/server/federation_client"; Gem::Server::FederationClient.new.subscribe(to_base_url: "http://localhost:9292")'
